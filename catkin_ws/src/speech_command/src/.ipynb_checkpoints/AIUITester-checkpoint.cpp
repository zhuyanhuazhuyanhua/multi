...
void setParams()
{
	char *setParams = "{\"audioparams\":{\"vcn\":\"x2_qige\"}}";
...
void TestListener::onEvent(const IAIUIEvent &event) const
{
	...
	case AIUIConstant::EVENT_RESULT: //data字段携带结果数据，info字段为描述数据的JSON字符串。
	{
		...
		Json::Value content = (data["content"])[0];//0
		...
		else if (sub == "nlp")
		{
			...
			if (NULL != buffer)
			{
				...
				if (resultStr.size() > 20)
				{
                    ...
                    Json::Value angle_awake = resultJson;
                    
					cout << ">>>>>角度是:" << angle_awake << endl;
#if if_print_proc_log
					...
					{
						res_question_str = "你好小飞";
						cout<<"1111----45555----------2222--3333--"<<endl;
						set_awake_word_once(res_question_str);//res_question_str
...
void gTTS(string text)
{
	if (NULL != globalAgent)
	{
		...
		string paramStr = "vcn=x2_qige"; //xiaoyan x2_pengfei x2_qige x2_yifei
		paramStr += ",speed=60";
		paramStr += ",pitch=50";
		paramStr += ",volume=80";
...
void process_recv(const unsigned char *buf, int len)
{
        ...
        if ((MsgType)pkg.type == MsgType::AIUI_MSG) 
	{
		...
		{
                    //Json::Value::Members mem = content["info"].getMemberNames();
                    //cout<<"mem: "<<mem.begin()<<endl;
                    //string angle_res =  content['info']['ivw']['angle'].asString();
                    //cout<<"angle_res: "<<content[0]<<endl;
                    string info = content["info"].asString();
                    //Json::Value ivw = info["ivw"];
		   // string arg1 = content["arg1"].asString();
      ...
      angle = root["ivw"]["angle"].asInt();
    cout<<"awake_angle: "<<angle<<endl;
      }
                    

                    //std::cout<<info["ivw"]["angel"].asInt()<<endl;
                  
                   // cout<<"ivw: "<<ivw<<endl;
            //cout<<"angle: "<<info["ivw"]["angle"].asInt()<<endl;
           // cout<<"angle: "<<info["ivw"]<<endl;
             //Json::Value root1;
             //Json::Reader reader1;
           // reader1.parse(content, root1);
               
                //std::cout<<root1["ivw"]["angel"].asInt()<<endl;
