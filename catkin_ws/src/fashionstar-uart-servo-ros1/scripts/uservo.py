...
class Packet(object):
	...
	HEADERS = ['\x12\x4c', '\x05\x1c']
	CODE_LEN = 1 # 功能编号长度
	SIZE_LEN = 1 # 字节长度
	CHECKSUM_LEN = 1 # 校验和长度

	@classmethod
	def calc_checksum(cls, code, param_bytes='', pkt_type=1):
		u'''计算校验和'''
		header = cls.HEADERS[pkt_type]
		checksum = sum(bytearray(header) + bytearray(struct.pack(u'<BB', code, len(param_bytes))) + bytearray(param_bytes)) %256
		# print("header: {}".format(header))
		# print("code: {}".format(code))
		# print("size: {}".format(len(param_bytes)))
	...
	def verify(cls, packet_bytes, pkt_type=1):
		...
		if packet_bytes[:cls.HEADER_LEN] != cls.HEADERS[pkt_type]:
			return False
		code, size = struct.unpack(u'<BB', packet_bytes[cls.HEADER_LEN : cls.HEADER_LEN + cls.CODE_LEN + cls.SIZE_LEN])
		...
		# 校验和检验
		checksum = bytearray(packet_bytes[-cls.CHECKSUM_LEN])[0]
		# logging.info('实际的Checksum : {} 计算得到的Checksum: {}'.format(checksum, cls.calc_checksum(code , param_bytes, pkt_type=pkt_type)))
		# print("checksum = {} Type of checksum ".format(checksum) + str(type(checksum)))
		# 校验和检查
	...
	@classmethod
	def pack(cls, code, param_bytes=''):
		u'''数据打包为二进制数据'''
		size = len(param_bytes)
		checksum = cls.calc_checksum(code, param_bytes, pkt_type=cls.PKT_TYPE_REQUEST)
		frame_bytes = cls.HEADERS[cls.PKT_TYPE_REQUEST] + bytearray(struct.pack(u'<BB', code, size)) + param_bytes + bytearray(struct.pack(u'<B', checksum))
		return frame_bytes
	...
	def unpack(cls, packet_bytes):
		...
		if not cls.verify(packet_bytes, pkt_type=cls.PKT_TYPE_RESPONSE):
			# 数据非法
			return None
		code = struct.unpack(u'<B', packet_bytes[cls.HEADER_LEN:cls.HEADER_LEN+cls.CODE_LEN])[0]
		param_bytes = packet_bytes[cls.HEADER_LEN + cls.CODE_LEN + cls.SIZE_LEN : -cls.CHECKSUM_LEN]
...
class PacketBuffer(object):
	...
	def update(self, next_byte):
		u'''将新的字节添加到Packet中转站'''
		# print '[INFO]: next byte: 0x%02x'%bytearray(next_byte)[0]
		if not self.header_flag:
			# 填充头部字节
			if len(self.header) < Packet.HEADER_LEN:
				# 向Header追加字节
				self.header += next_byte
				# print "cur header[0]"  +  "0x%02x"%bytearray(self.header)[0]
				# if len(self.header) > 1:
				# 	print "cur header[1]"  +  "0x%02x"%bytearray(self.header)[1]
				
				# print "target header[0]"  +  "0x%02x"%bytearray(Packet.HEADERS[Packet.PKT_TYPE_RESPONSE])[0]
				# print "target header[1]"  +  "0x%02x"%bytearray(Packet.HEADERS[Packet.PKT_TYPE_RESPONSE])[1]
			...
			if len(self.size) < Packet.SIZE_LEN:
				...
				if len(self.size) == Packet.SIZE_LEN:
					self.size_flag = True
					# 更新参数个数
					self.param_len = int(bytearray(struct.unpack(u'<B', self.size))[0])
					# print("param len : {}".format(self.param_len))
...
class UartServoInfo(object):
	...
	def __str__(self):
		return u"目标角度:{:.1f} 实际角度:{:.1f} 角度误差:{:.2f}".format(self.target_angle, self.angle, self.target_angle-self.angle)

class UartServoManager(object):
	u'''串口舵机管理器'''
	...
	def send_request(self, code, param_bytes):
		...
		try:
			...
			logging.error(u'串口数据发送异常, 请检查是否是USB口松动或设备号变更, 需重新初始化机械臂')
			# TODO 添加掉线检查

	def ping(self, servo_id):
		u'''发送Ping请求'''
		# self.cur_ping_servo_id = servo_id #　为了可视化显示
		self.send_request(self.CODE_PING, (struct.pack(u'<B', servo_id)))
		if self.is_debug:
			logging.info(u'PING 舵机 id={}'.format(servo_id))
		self.update(wait_response=True)
		ret = servo_id in self.servos
		if self.is_debug and ret:
			logging.info(u'[fs_uservo]串口舵机ID={} 响应ping'.format(servo_id))
		if ret:
	  		# 更新舵机角度
	...
	def scan_servo(self, srv_num=254):
		u'''扫描所有的舵机'''
		# ping所有的舵机
		for servo_id in xrange(srv_num):
			# 发送ping请求
			self.ping(servo_id)
		if self.is_debug:
			logging.info(u"有效的舵机ID列表: {}".format(list(self.servos.keys())))
	
	def response_ping(self, param_bytes):
		u'''响应PING请求'''
		servo_id, = struct.unpack(u'<B', param_bytes)
		if servo_id not in self.servos:
			self.servos[servo_id] = UartServoInfo(servo_id)
			self.servos[servo_id].is_online = True # 设置舵机在线的标志位
			if self.is_debug:
				logging.info(u'[fs_uservo]ECHO 添加一个新的舵机 id={}'.format(servo_id))
		else:
			self.servos[servo_id].is_online = True # 设置舵机在线的标志位
			if self.is_debug:
				logging.info(u'[fs_uservo]ECHO 已知舵机 id={}'.format(servo_id))
		

	def query_servo_angle(self, servo_id):
		u'''更新单个舵机的角度'''
		if self.is_debug:
			logging.info(u'查询单个舵机的角度 id={}'.format(servo_id))
		# print("查询舵机角度 多圈模式? {}".format(self.servos[servo_id].is_mturn))
		if self.servos[servo_id].is_mturn:
			# 多圈模式
			self.send_request(self.CODE_QUERY_SERVO_ANGLE_MTURN, (struct.pack(u'<B', servo_id)))
		else:
			# 单圈模式
			self.send_request(self.CODE_QUERY_SERVO_ANGLE, (struct.pack(u'<B', servo_id)))
	...
	def response_query_servo_angle(self, param_bytes):
		u'''响应查询单个舵机角度'''
		# 数据解包
		servo_id, angle = struct.unpack(u'<Bh', param_bytes)
		# 舵机的分辨率是0.1度
		angle /= 10
		...
		if servo_id not in self.servos:
			...
			if self.is_debug:
				logging.info(u'[INFO] 更新舵机角度 id={}  角度: {:.2f} deg'.format(servo_id, angle))
	def response_query_servo_angle_mturn(self, param_bytes):
		u'''响应舵机角度查询角度(多圈)'''
		# 数据解包
		servo_id, angle, mturn = struct.unpack(u'<Bih', param_bytes)
  		# 舵机的分辨率是0.1度
		angle /= 10.0
	...
	def refresh_srv_list(self, max_servo_id=254):
		...
		# 清空已有的字典
		self.servos = {}
		for servo_idx in xrange(max_servo_id):
			self.ping(servo_idx)
			for ti in xrange(20):
				# 查询一个舵机最多等待1000ms
				self.update()
	...
	def query_srv_info(self, servo_id):
		u'''查询单个舵机的所有配置'''
		self.send_request(self.CODE_QUERY_SERVO_INFO, (struct.pack(u'<B', servo_id)))
	...
	def set_servo_angle(self, servo_id, angle, is_mturn=False, interval=None, velocity=None, t_acc=20, t_dec=20,  power=0, mean_dps=100.0):
		...
		'''
		if servo_id not in self.servos:
			logging.warn(u'未知舵机序号: {}'.format(servo_id))
			return False
		# 同步修改srv_info
		...
		if interval is not None and interval != 0:
			...
			if is_mturn:
				if interval < t_acc + t_dec:
					...
					interval = 4096000
				param_bytes = (struct.pack(u'<BiIHHH', servo_id, angle, interval, t_acc, t_dec, power))
				self.send_request(self.CODE_SET_SERVO_ANGLE_MTURN_BY_INTERVAL, param_bytes)
			else:
				param_bytes = (struct.pack(u'<BhHHHH', servo_id, angle, interval, t_acc, t_dec, power))
			...
			if is_mturn:
				param_bytes = (struct.pack(u'<BiHHHH', servo_id, angle, velocity, t_acc, t_dec, power))
				self.send_request(self.CODE_SET_SERVO_ANGLE_MTURN_BY_VELOCITY, param_bytes)
			else:
				param_bytes = (struct.pack(u'<BhHHHH', servo_id, angle, velocity, t_acc, t_dec, power))
			...
			if is_mturn:
				param_bytes = (struct.pack(u'<BiIH', servo_id, angle, interval, power))
				self.send_request(self.CODE_SET_SERVO_ANGLE_MTURN, param_bytes)
			else:
				param_bytes = (struct.pack(u'<BhHH', servo_id, angle, interval, power))
	...
	def set_wheel(self, servo_id, mode, value=0, is_cw=True, mean_dps=None):
		...
		# 发送请求
		self.send_request(self.CODE_SET_SPIN, (struct.pack(u'<BBHH', servo_id, method, mean_dps, value)))
	...
	def set_damping(self, servo_id, power=0):
		...
		@param power
			舵机保持功率
		'''
		self.send_request(self.CODE_SET_DAMPING, (struct.pack(u'<BH', servo_id, power)))
	
	def reset_user_data(self, servo_id):
		u'''用户表数据重置'''
		# 发送请求数据
		self.send_request(self.CODE_RESET_USER_DATA, (struct.pack(u'<B', servo_id)))
		# 等待数据回传
		# 注: 舵机并不会回传指令
		# self.update(wait_response=True)
		return True
	...
	def response_reset_user_data(self, param_bytes):
		u'''用户数据重置响应'''
		servo_id, result = struct.unpack(u'<BB', param_bytes)
		if self.is_debug:
			# logging.info("舵机用户数据重置 舵机ID={} 是否成功={}".format(servo_id, result))
			print u"舵机用户数据重置 舵机ID={} 是否成功={}".format(servo_id, result)
	...
	def read_data(self, servo_id, address):
		...
		'''
		# 发送请求
		self.send_request(self.CODE_READ_DATA, (struct.pack(u'<BB', servo_id, address)))
		self.update(wait_response=True) # 等待数据回传
		if self.is_debug:
			logging.info(u"READ DATA 舵机ID={} Address={}".format(servo_id, address))
			logging.info(u"DATA : {}".format(self.servos[servo_id].data_table_raw_dict[address]))
		# 返回读取到的数据
	...
	def response_read_data(self, param_bytes):
		u'''处理内存表数据读取回传'''
		# 数据提取
		servo_id, address = struct.unpack(u'<BB', param_bytes[:2])
	...
	def write_data(self, servo_id, address, content):
		u'''写入数据'''
		# 发送请求
		self.send_request(self.CODE_WRITE_DATA, (struct.pack(u'<BB', servo_id, address)+content))
		# 初始化标志位
		self.servos[servo_id].data_write_success = False
		if self.is_debug:
			logging.info(u"WRITE DATA 舵机ID={} Address={} Value={}".format(servo_id, address, content))
		# 等待数据回传
		self.update(wait_response=True) 
	...
	def update(self, is_empty_buffer=False, wait_response=False, timeout=0.02):
		...
		while True:
			...
			if len(buffer_bytes) != 0:
				if self.is_debug:
					logging.info(u'Recv Bytes: ')
					logging.info(u' '.join([u'0x%02x'%b for b in bytearray(buffer_bytes)]))

			# 将读入的Bytes打包成数据帧
			if buffer_bytes is not None:
				for b in buffer_bytes:
					# print "Type of b : " + str(type(b))
					self.pkt_buffer.update(struct.pack(u'<B', bytearray(b)[0]))
		...
		while self.pkt_buffer.has_valid_packet():
			...
			if code in self.response_handle_funcs:
				...
				logging.warn(u'未知功能码 : {}'.format(code))
